...
public class MinionMonster : MonoBehaviour{
	...
	private void OnCollisionEnter(Collision collision)
	{
		if(collision.transform.tag == "Bullet")
		{
			...
			for (int i = 0; i < drawBar.GetSliderMListHPber().Count; i++)
			{
				if (minionMonster.basecharacterinformation.IsDie(i))
				{

					Destroy(drawBar.GetMHeadUpPosition(i).gameObject);
					Destroy(drawBar.GetMonsterHPber(i).gameObject);
					Destroy(gameObject);

					drawBar.RemoveMonsterList(i);
				}
			}
